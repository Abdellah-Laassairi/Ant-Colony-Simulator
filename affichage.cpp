/*************************************************************
           ...
           Fonctions permettant de gérer l'affichage
              des fourmis et du monde environnant
                 (affichage effectue sous X11)

    ------------------------------------------------------

 Auteur: HILAIRE Thibault, BRIDAY Mikaël
 Date: 01/10/2003
 ...
 But: prototypes des fonctions permettant de gérer l'affichage
      ...
      et obstacles) et des phéromones
...
#include <cstdio>			// pour l'affichage des erreurs
#include <ctime>			// pour initialiser le générateur aléatoire avec l'heure courante
#include <cstdlib>         // pour srand et malloc
#include <unistd.h>
#include "constantes.h" 		//défini NOURRITURE_MAX

#include "affichage.h"		// déclaration des fonctions

// constantes (pour le graphique)
const int CASE = 7;
const int FOURMI = 2;
const int INDICE_COULEUR_PHERO_MAX = 100;
...
//t_pos fourmiliere;	// position de la fourmiliére
...
//          (fonction trés utile quand je testais une phéromone compris entre -PHEROMONES_MAX et +PHEROMONES_MAX)
int phero2indice( int phero, int x, int y)
{
	if ( phero<0 )
	{
		fprintf(stderr, "Warning (MiseAJourPheromones) : l'indice de phéromone ne peut être négatif (x=%d y=%d phero=%d)\n",x,y,phero);
		phero=0;
	}
	if (phero>PHEROMONES_MAX)
	{
		fprintf(stderr, "Warning (MiseAJourPheromones) : l'indice de phéromone ne peut être supérieur à %d (x=%d y=%d phero=%d)\n",PHEROMONES_MAX,x,y,phero); 
...
//principe: création de la fenêtre X11, initialisation des couleurs, des pixmap, etc...
...
//                  0 : la fourmi est en mode "j'ai trouvé de la nourriture, je retourne à la fourmilliére"
//                  1 : la fourmi est en mode "je suis en recherche de nourriture"
//but: mettre à jour la fourmi dans le monde
//principe: on stocke les paramètres de la fourmi dans une liste, et on l'affichera dans Affichage
//          (on ne peut l'afficher qu'aprés avoir affiché la nourriture et la phéromone, et on en profite pour toutes les afficher en meme temps)
void MiseAJourFourmi( int x, int y, int recherche)
{
	// vérifications
	if (!init) fprintf(stderr, "Warning (MiseAJourFourmi) : InitAffichage n'a pas encore été exécuté\n");
	if ( (x<0) || (x>=env.L) || (y<0) || (y>=env.H) )
		fprintf(stderr, "Warning (MiseAJourFourmi) : coordonnées de fourmi incorrecte (x=%d  y=%d)\n",x,y);
	if (env.mat[y][x]==OBSTACLE)
		fprintf(stderr, "Warning (MiseAJourFourmi) : une fourmi est sur un obstacle (x=%d  y=%d)\n",x,y);

    // Création et ajout de la fourmi
    t_lpos* nf = new t_lpos;
    nf->pos.x = x;
    nf->pos.y = y;
...
//principe: stocke la position de la fourmilliére dans une liste
//          celle-si sera en fait affichée pendant Affichage
// (on fait ça, parce que les fourmis déposent de la nourriture sur la fourmilliére
// et qu'il est plus simple de la réafficher à chaque fois, que de vérifier qu'on affiche
// pas de la nourriture sur une fourmilliére)
/*void MiseAJourFourmiliere(int x, int y)
{
	// vérifications
	if (!init)
		fprintf(stderr, "Warning (MiseAJourFourmiliere) : InitAffichage n'a pas encore été exécuté\n");
	if ( (x<0) || (x>env.L) || (y<0) || (y>env.H) )
		fprintf(stderr, "Warning (MiseAJourFourmiliere) : coordonnees incorrectes (x=%d  y=%d)\n",x,y);
	if (FourmiliereDefinie)
		fprintf(stderr, "Warning (MiseAJourFourmiliere) : On ne peut appeler qu'une fois MiseAJourFourmilliére\n");
...
//action: MiseAJourEnvironnement
//argument: environnement : t_monde contenant l'environnement (obstacles et nourriture)
//but: mettre à jour l'environnement en cours (obstacles en bleu et nourriture en vert : un point vert au milieu, et un rectangle dont l'intensité
...
void MiseAJourEnvironnement(const t_monde & environ)
{
	int i,j;
	// vérifications
	if (!init)
		fprintf(stderr, "Warning (MiseAJourEnvironnement) : InitAffichage n'a pas encore été exécuté\n");

	if ( (env.H!=environ.H) || (env.L!=environ.L) )
	{
		fprintf(stderr, "Warning (MiseAJourEnvironnement) : la taille de l'environnement passé en argument d'AfficheEnvironnemnt n'a pas la même taille celle donnée à la fonction InitAffichage\n");
	...
	for(i=0;i<env.H;i++)
		for(j=0;j<env.L;j++)
		{
			...
			else if ( (environ.mat[i][j]<=NOURRITURE_MAX) && (environ.mat[i][j]>VIDE) )
			{
				...
				XFillRectangle( display,pix,gc, j*CASE+3,i*CASE+3, FOURMI, FOURMI);
			}
			else
			{
				if ( (i!=environ.Fy) && (j!=environ.Fx) && (environ.mat[i][j]!=VIDE) )
				fprintf(stderr, "Warning (MiseAJourEnvironnement) : le tableau environnement contient d'autres valeurs que OBSTACLE, VIDE  ou bien une quantite de nourriture supérieure à %d\n", NOURRITURE_MAX);
			}
			env.mat[i][j] = environ.mat[i][j];
		}

	// copie la fourmiliére sur pix
...
//          (on n'affiche pas de phéromones ailleurs, ni sur les obstacles, la nourriture ou la fourmilliére)
void MiseAJourPheromones(const t_matrice & phero)
{
	int i,j;
	// vérification
	if (!init)
		fprintf(stderr, "Warning (MiseAJourPheromones) : InitAffichage n'a pas encore été exécuté\n");

	// parcourt du tableau
	for(i=0;i<env.H;i++)
		for (j=0;j<env.L;j++)
		{
			if ( (env.mat[i][j]==VIDE) && ((i!=env.Fy) || (j!=env.Fx)) )
			{
				XSetForeground( display, gc, CoulPheromones[ phero2indice(phero[i][j],j,i) ]);
				XFillRectangle( display,pix,gc, j*CASE+1,i*CASE+1, CASE-1, CASE-1);
			}
		}
}
...
//     nécessaire pour gérer les événement graphiques (déplacement de fenêtre, touche pressée au clavier, etc...)
...
//          une boucle d'attente (delai fixé par argument) permet de gérer les événements/signaux qui peuvent intervenir : fenêtre cachée ou
//          réaffichée, touche pressée, etc...
int Affichage()
{
	static int nb_iter = 1;
	static uint tempoAffichage = 10; //en us.

	// vérifications
	if (!init)
		fprintf(stderr, "Warning (Affichage) : InitAffichage n'a pas encore été exécuté\n");
	...
	time_t time=clock();
	XEvent e;
	while (XPending(display))		// y'a-t-il un event qui nous attend
	{
		...
		switch (e.type)
		{
			...
			case KeyPress:			// touche pressée
				if (XLookupKeysym(&e.xkey, 0) == XK_KP_Add)	// ('+') plus vite
				{
					tempoAffichage *=1.1; //+10%
				}
				else if (XLookupKeysym(&e.xkey, 0) == XK_KP_Subtract)	// ('-') moins vite
				{
					tempoAffichage *=.9; //-10%
				}
				else if (XLookupKeysym(&e.xkey, 0) == XK_Escape)
				{
					...
					// teste si la fourmiliére est correctement placée
					if ( (env.Fx<0) || (env.Fx>env.L) || (env.Fy<0) || (env.Fy>env.H) )
					{
						fprintf(stderr, "Warning (Affichage) : la fourmiliére est placée en (%d, %d)\n", env.Fx, env.Fy);
					}
					else
					{
						// affiche les conclusions : nourriture par fourmilliére
						printf("Simulation en %d itérations\n", nb_iter); 
						printf("Nourriture rapportée à la fourmilliére = %d\n", env.mat[env.Fy][env.Fx]);
					}
					return 0;
				}
				break;
	...
	const int attenteUS =  ((clock()-time)*1000000/CLOCKS_PER_SEC);
	if(attenteUS < tempoAffichage) usleep(tempoAffichage-attenteUS);
	usleep(100);
